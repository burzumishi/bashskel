...
# libbsk.la: Core functions and definitions #
...
main() {
	...
	do
		...
		check_directory_structure "$LOCKDIR $TMPDIR";
		...
		if test -z "$*"; then
			...
			ACTION_OPTIONS=$($GETOPT -o hv \
				 -l "help,version,verbose,force,color" \
				 -n "$0" -- "$@");
...
check_file() {
  ...
  if test ! -f "$FILE"; then
    echolog_debug "$DEBUG Warning! Requested file '$FILE' was not found!";
		...
		# Define $BSK_LAST_ERROR to be displayed in _(errorcheck)_ #
		BSK_LAST_ERROR="$WARNING Warning! Requested file '$FILE' was not found!";
...
check_dir() {
	...
	# Test $DIR #
		...
		# Define $BSK_LAST_ERROR to be displayed in _(errorcheck)_ #
		BSK_LAST_ERROR="$WARNING Warning! Requested directory '$DIR' was not found!";
...
check_link() {
  ...
  if test ! -L "$LINK"; then
    echolog_debug "$DEBUG Warning! Requested file link '$LINK' was not found!";
		...
		# Define $BSK_LAST_ERROR to be displayed in _(errorcheck)_ #
		BSK_LAST_ERROR="$WARNING Warning! Requested file link '$LINK' was not found!";
